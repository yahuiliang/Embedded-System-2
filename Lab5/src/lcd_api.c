...
static void init_PB0_to_PB2_mode() {
	for (int bit = 0; bit < 3; bit++) {
		gpiob -> MODER &= ~(0b11 << (bit * 2)); // make sure all bits are set to zero.
		gpiob -> MODER |= OUTPUT << (bit * 2); // set mode from PB0 to PB2 to output mode.
	}
}
...
static void init_PC8_to_PC11_mode() {
	for (int bit = 8; bit < 12; bit++) {
		gpioc -> MODER &= ~(0b11 << (bit * 2)); // make sure all bits are set to zero.
		gpioc -> MODER |= OUTPUT << (bit * 2); // set mode from PC7 to PC11 to output mode.
	}
}
